...
subroutine mksoiltex_pio(ldomain_pio, mapfname, datfname, ndiag, sand_o, clay_o)
  ...
  character(len=38)      :: soil(0:nlsm)           ! name of each soil texture
  real(r8)               :: wt                     ! map overlap weight
  integer                :: l,k,no,ns_loc_i        ! indices
  integer                :: k1,k2                  ! indices
  integer                :: ier                    ! error status
  integer                :: miss = 99999           ! missing data indicator
  logical                :: found                  ! temporary
  ...
  !
  ! Get 'frac_src' locally
  !
  PetscCallA(ISCreateBlock(PETSC_COMM_WORLD, 1, num_nonzero_wts, ja_ptr, PETSC_COPY_VALUES, is_from, ierr))

  PetscCallA(VecCreate(PETSC_COMM_SELF, frac_src_dst_vec, ierr))
  PetscCallA(VecSetSizes(frac_src_dst_vec, num_nonzero_wts, PETSC_DECIDE, ierr))
  PetscCallA(VecSetFromOptions(frac_src_dst_vec, ierr))

  allocate(int_array(num_nonzero_wts))
  do i = 1, num_nonzero_wts
    int_array(i) = i - 1
  enddo
  PetscCallA(ISCreateBlock(PETSC_COMM_WORLD, 1, num_nonzero_wts, int_array, PETSC_COPY_VALUES, is_to, ierr))
  deallocate(int_array)

  PetscCallA(VecScatterCreate(tgridmap_pio%src_vec, is_from, frac_src_dst_vec, is_to, vec_scatter, ierr))
  ...
  !
  ! Get 'MAPUNITS' locally using the VecScatter created previously
  !
  PetscCallA(VecDuplicate(frac_src_dst_vec, mapunit_dst_vec, ierr))
  ...
  ! Perform the VecScatter
  PetscCallA(VecScatterBegin(vec_scatter, tgridmap_pio%src_vec, mapunit_dst_vec, INSERT_VALUES, SCATTER_FORWARD, ierr))
  PetscCallA(VecScatterEnd(vec_scatter, tgridmap_pio%src_vec, mapunit_dst_vec, INSERT_VALUES, SCATTER_FORWARD, ierr))

  ! Clean up
  PetscCallA(VecScatterDestroy(vec_scatter, ierr))

  !
  ! Get the weights locally
  !

  ! create Vecs
  ns_len = tgridmap_pio%dim_ns%endd - tgridmap_pio%dim_ns%begd + 1
  PetscCallA(VecCreate(PETSC_COMM_WORLD, wts_glb_vec, ierr))
  PetscCallA(VecSetSizes(wts_glb_vec, ns_len, PETSC_DECIDE, ierr))
  ...
  allocate(int_array(num_nonzero_wts))
  do i = 1, num_nonzero_wts
    int_array(i) = i - 1 + offset
  enddo
  PetscCallA(ISCreateBlock(PETSC_COMM_WORLD, 1, num_nonzero_wts, int_array, PETSC_COPY_VALUES, is_from, ierr))
  deallocate(int_array)
  ...
  PetscCallA(VecScatterCreate(wts_glb_vec, is_from, wts_dst_vec, is_to, vec_scatter, ierr))
  PetscCallA(ISDestroy(is_from, ierr))
  PetscCallA(ISDestroy(is_to, ierr))

  PetscCallA(VecScatterBegin(vec_scatter, wts_glb_vec, wts_dst_vec, INSERT_VALUES, SCATTER_FORWARD, ierr))
  PetscCallA(VecScatterEnd(vec_scatter, wts_glb_vec, wts_dst_vec, INSERT_VALUES, SCATTER_FORWARD, ierr))
  PetscCallA(VecScatterDestroy(vec_scatter, ierr))
